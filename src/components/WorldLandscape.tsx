...
import { CheckCircle2, Lock, Play, Flame, Sparkles, Mountain, Trees, Compass, Sun } from 'lucide-react';
...
export const WorldLandscape: React.FC<WorldLandscapeProps> = ({
  ...
  // Overworld stage coordinates along the winding pilgrimage path
  const nodePositions: NodePoint[] = [
    { x: 25, y: 78 }, // Stage 1 (Valley / Foothills)
    { x: 74, y: 58 }, // Stage 2 (Mid-Elevation / Ridge)
    { x: 30, y: 38 }, // Stage 3 (High Plateau / Pass)
    { x: 50, y: 16 }  // Sanctuary Checkpoint (Campfire / Summit Sanctuary)
  ];
  ...
  return (
    <div className="relative w-full h-[410px] rounded-2xl overflow-hidden border-2 border-gold/40 shadow-[0_12px_32px_rgba(0,0,0,0.8)] bg-[#0a0c16] select-none">
      {/* Biome Switcher Bar (Top Right) */}
      <div className="absolute top-2.5 right-2.5 z-40 flex items-center bg-black/75 backdrop-blur-md rounded-xl p-1 border border-white/10 shadow-lg space-x-1">
        <button
          onClick={() => { audio.playFootstep(); setSelectedBiome('snow'); }}
          className={`px-2 py-1 rounded-lg text-[10px] flex items-center space-x-1 transition-all ${
            selectedBiome === 'snow' ? 'bg-blue-600/80 text-white font-bold shadow' : 'text-gray-400 hover:text-white'
          }`}
          title="Snowy Mountains"
        >
          <span>❄️</span>
          <span className="hidden xs:inline">Snow</span>
        </button>

        <button
          onClick={() => { audio.playFootstep(); setSelectedBiome('savannah'); }}
          className={`px-2 py-1 rounded-lg text-[10px] flex items-center space-x-1 transition-all ${
            selectedBiome === 'savannah' ? 'bg-amber-600/80 text-white font-bold shadow' : 'text-gray-400 hover:text-white'
          }`}
          title="Golden Savannah"
        >
          <span>🌾</span>
          <span className="hidden xs:inline">Savannah</span>
        </button>

        <button
          onClick={() => { audio.playFootstep(); setSelectedBiome('desert'); }}
          className={`px-2 py-1 rounded-lg text-[10px] flex items-center space-x-1 transition-all ${
            selectedBiome === 'desert' ? 'bg-yellow-600/80 text-white font-bold shadow' : 'text-gray-400 hover:text-white'
          }`}
          title="Sun Desert"
        >
          <span>🏜️</span>
          <span className="hidden xs:inline">Desert</span>
        </button>

        <button
          onClick={() => { audio.playFootstep(); setSelectedBiome('forest'); }}
          className={`px-2 py-1 rounded-lg text-[10px] flex items-center space-x-1 transition-all ${
            selectedBiome === 'forest' ? 'bg-emerald-700/80 text-white font-bold shadow' : 'text-gray-400 hover:text-white'
          }`}
          title="Ancient Forest"
        >
          <span>🌲</span>
          <span className="hidden xs:inline">Forest</span>
        </button>
      </div>
      ...
      {selectedBiome === 'snow' && (
        <div className="absolute inset-0 bg-gradient-to-b from-[#0b1329] via-[#132247] to-[#1e346b]">
          ...
          <div className="absolute left-10 top-12 w-1.5 h-1.5 bg-white rounded-full shadow-[0_0_6px_#ffffff] animate-bounce" style={{ animationDuration: '3.5s' }} />
          <div className="absolute right-16 top-20 w-2 h-2 bg-white rounded-full shadow-[0_0_8px_#ffffff] animate-bounce" style={{ animationDuration: '4.2s' }} />
          <div className="absolute left-1/3 top-28 w-1 h-1 bg-white rounded-full animate-pulse" />
          <div className="absolute right-1/3 top-16 w-1.5 h-1.5 bg-blue-100 rounded-full animate-bounce" style={{ animationDuration: '5.1s' }} />
        </div>
      )}

      {/* ========================================================= */}
      {/* 2. TERRAIN: GOLDEN SAVANNAH (Fellowship & Warm Twilight) */}
      {/* ========================================================= */}
      {selectedBiome === 'savannah' && (
        <div className="absolute inset-0 bg-gradient-to-b from-[#2e1065] via-[#701a75] to-[#b45309]">
          {/* Massive Golden Setting Sun */}
          ...
          <svg className="absolute bottom-0 w-full h-full" viewBox="0 0 400 410" preserveAspectRatio="none">
            ...
            {/* Ancient Baobab Silhouette on Right */}
            <g fill="#2e1065">
          ...
          {/* Evening Bird Silhouettes migrating in sunset */}
      ...
      {selectedBiome === 'desert' && (
        <div className="absolute inset-0 bg-gradient-to-b from-[#1c1917] via-[#451a03] to-[#78350f]">
          ...
          <svg className="absolute bottom-0 w-full h-full" viewBox="0 0 400 410" preserveAspectRatio="none">
            ...
            {/* Ancient Crumbling Temple Monoliths */}
          ...
          {/* Oasis Palm Trees */}
          <div className="absolute left-[165px] top-[305px] text-2xl select-none">
            🌴
          </div>
          <div className="absolute left-[215px] top-[312px] text-xl select-none">
            🌴
          </div>

          {/* Drifting Golden Dust Motes */}
          <div className="absolute left-1/4 top-36 w-1.5 h-1.5 bg-[#fef08a] rounded-full opacity-60 animate-ping" style={{ animationDuration: '4s' }} />
          <div className="absolute right-1/4 top-48 w-1 h-1 bg-[#fef08a] rounded-full opacity-80 animate-ping" style={{ animationDuration: '3s' }} />
        </div>
      )}

      {/* ========================================================= */}
      {/* 4. TERRAIN: DEEP ANCIENT FOREST (Sacred Living Waters) */}
      {/* ========================================================= */}
      {selectedBiome === 'forest' && (
        <div className="absolute inset-0 bg-gradient-to-b from-[#022c22] via-[#064e3b] to-[#065f46]">
          {/* Dappled Sunbeams / God-Rays */}
          <div
          ...
          {/* Forest Canopy & Ancient Redwood Trunks */}
          <svg className="absolute bottom-0 w-full h-full" viewBox="0 0 400 410" preserveAspectRatio="none">
            {/* Back Tree Trunks */}
          ...
          <div className="absolute left-1/2 top-48 w-2 h-2 rounded-full bg-[#67e8f9] shadow-[0_0_10px_#22d3ee] animate-pulse-gentle" />
          <div className="absolute right-1/3 top-24 w-1.5 h-1.5 rounded-full bg-[#fbcfe8] shadow-[0_0_8px_#f472b6] animate-pulse" />
        </div>
      )}

      {/* ========================================================= */}
      {/* COBBLESTONE PILGRIMAGE TRAIL (Winding through terrain) */}
      {/* ========================================================= */}
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-10" viewBox="0 0 100 100" preserveAspectRatio="none">
        {/* Soft Trail Shadow */}
        <path
          d={`M ${nodePositions[0].x} ${nodePositions[0].y} Q 50 68, ${nodePositions[1].x} ${nodePositions[1].y} T ${nodePositions[2].x} ${nodePositions[2].y} T ${nodePositions[3].x} ${nodePositions[3].y}`}
          fill="none"
          stroke="rgba(0, 0, 0, 0.7)"
          strokeWidth="3.2"
        />
        {/* Luminous Golden Cobblestone Path */}
        <path
          d={`M ${nodePositions[0].x} ${nodePositions[0].y} Q 50 68, ${nodePositions[1].x} ${nodePositions[1].y} T ${nodePositions[2].x} ${nodePositions[2].y} T ${nodePositions[3].x} ${nodePositions[3].y}`}
          fill="none"
          stroke="#fde047"
      ...
      {/* ========================================================= */}
      {/* STAGE NODES ALONG THE OVERWORLD TRAIL */}
      {/* ========================================================= */}
